...
`;
  ...
  // 1) onPointerMove: hover‐only highlight
  function handlePointerMove(
    ...
    if (pick.hit && pick.pickedMesh) {
      ...
      highlightLayer.addMesh(pick.pickedMesh, BABYLON.Color3.White());
      canvas.style.cursor = "pointer";

      // position & show your tooltip
      tooltip.innerText = pick.pickedMesh.name;
  ...
  // 2) onPointerPick: click‐only selection + highlight
  function handlePointerPick(pointerInfo, scene, highlightLayer) {
    ...
    if (pick.hit && pick.pickedMesh) {
      highlightLayer.addMesh(pick.pickedMesh, BABYLON.Color3.Red());

      /* your existing selection logic … */
      const partName = getPartNameFromMeshName(pick.pickedMesh.name);
      handlePartSelection(partName, pick.pickedMesh.name);
    }
  }
  ...
  const createScene = () => {
    ...
    // hook up our 2 global pointer observers
    scene.onPointerObservable.add((info) =>
      handlePointerMove(info, scene, highlightLayer, canvas, tooltip)
    );
    scene.onPointerObservable.add((info) =>
      handlePointerPick(info, scene, highlightLayer)
    );
    ...
    BABYLON.SceneLoader.ImportMesh("", "./", "jacket.glb", scene, (meshes) => {
      meshes.forEach((mesh) => {
        ...
        // Check for the collar mesh; note the global variable is used here.
        if (mesh.name.toLowerCase().includes("collar")) {
          collarMesh = mesh;
        }
  ...
  function setupPartHoverHighlight() {
    ...
    partOptionButtons.forEach((button) => {
      button.addEventListener("mouseenter", function () {
        ...
        // When hovering lapels, add collar highlight if available
        if (partName === "Lapels" && collarMesh) {
          highlightLayer.addMesh(collarMesh, BABYLON.Color3.White());
        }
      });
      ...
      button.addEventListener("mouseleave", function () {
        ...
        // When no longer hovering lapels, remove collar highlight
        if (partName === "Lapels" && collarMesh) {
          highlightLayer.removeMesh(collarMesh);
        }
      });
    });
...
`;
  ...
  function showMobileLapelsOptions() {
    ...
    confirmButton.addEventListener("click", showJacketDesignOptions);
    textureContainer.appendChild(confirmButton);

    // STATIC LIST (no Flickity)
    const mobileLapelsSlider = document.createElement("div");
    mobileLapelsSlider.id = "mobileLapelsSlider";
    // we’ve intentionally _not_ added "cards-wrapper" here
    // so your global slider-initer won’t touch it
    ...
    lapelsDesigns.forEach((item) => {
      ...
      lapelCard.innerHTML = `
      <div class="img-wrapper" style="touch-action: pan-y;">
        <img src="${item.src}" alt="${item.label}" style="width:100%;height:auto;touch-action: pan-y;">
      </div>
      <p style="touch-action: pan-y;">${item.label}</p>
    `;
      ...
      mobileLapelsSlider.appendChild(lapelCard);
    });
  ...
  function showMobileBackOptions() {
    ...
    backViewOptions.forEach((item) => {
      ...
      backCard.classList.add("part-option", "card_cardContainer");
...
<a id="confirm-blazer-sizes" class="confirm-size-btn">Confirm</a>
          ...
          textureContainer.innerHTML = `
            ...
            <div class="panel" style="max-height: 0px;">
              ${generatePartItems([
                ...
                { partName: "Buttons", options: partOptions.Buttons }, // ← new line
              ])}
            </div>
      ...
      .map((part) => {
        ...
        if (part.partName === "Pockets") {
          ...
          const bottomPockets = part.options.filter((meshName) =>
            BOTTOM_POCKETS.includes(meshName)
          );
  ...
  function switchPartMesh(partName, meshName, mobileSelection) {
    ...
    if (partName !== "Pockets") {
      ...
      if (selectedMesh) {
        ...
        // ——— NEW: if lapels are selected, also keep the collar highlighted ———
        if (partName === "Lapels" && collarMesh) {
          highlightLayer.addMesh(collarMesh, BABYLON.Color3.White());
        }
      } else {
  ...
  function initializeCardsSlider() {
    const cardsWrappers = document.querySelectorAll(
      ".cards-wrapper:not(.design-options):not(#chooseGarmentContainer):not(#mobilePocketsContainer)"
    );
