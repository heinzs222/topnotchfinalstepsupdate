...
`;
  ...
  function validateMeasurements() {
    // Combined required measurements for both pants and blazer
    ...
    // Loop through each required measurement
    for (let measurement of requiredMeasurements) {
      const value = userChoices.measurements[measurement];
      // For a select element, check if a value exists and is not empty
      if (measurement === "bodyTypeSelect") {
        if (!value || value.trim() === "") {
          return false;
        }
      } else {
        // For numeric inputs, check that the value is defined and greater than 0
